	...
	if (popupWindow !== null) {
		...
		if (activeInfo["type"] == "T") {
			...
			if (findProgramList.length > 0) {
				...
				//미등록 메뉴인 경우 임시로 소스 xml 의 파일명으로 지정 (임시) - 2024.03.25
				activeInfo["programCd"] = gcm.win.getProgramId($p.main().tac_layout.getWindow(selectedTabId).$p);
			}
			...
			if (findProgramList.length > 0) {
				...
				//미등록 메뉴인 경우 임시로 소스 xml 의 파일명으로 지정 (임시) - 2024.03.25
				activeInfo["programCd"] = gcm.win.getProgramId($p.main().wdc_main.getWindow(selectedWindowId).$p);
			}
			...
			if (!com.util.isEmpty(gcm.data.getParameter($p.main().wfm_layout.getWindow().$p, 'menuInfo').programCd)) {
				...
				//미등록 메뉴인 경우 임시로 소스 xml 의 파일명으로 지정 (임시) - 2024.03.25 
				activeInfo["programCd"] = gcm.win.getProgramId($p.main().wfm_layout.getWindow().$p);
			}
...
gcm.win.openMenu = function($p, menuNm, url, menuCode, paramObj, option) {
	...
	if (com.util.isEmpty(url)) {
		...
		if ((typeof paramObj !== "undefined") && (paramObj !== null)) {
			...
			refresh = true;		//파라미터 존재 시 refresh 되도록...(2024.03.25) 
		}
		
		//programCd 추가 (2024-02-16) 
		var programCd = gcm.menuComp.getMatchedColumnData("SRC_PATH", url, "PROGRAM_CD")[0] || "";
		
		//등록된 메뉴가 아닐 경우 url 에서 임시 programCd 추출하여 사용 ??? (사용시 검토가 필요함)  & gcm.win.getActiveWindow() 에서도 동일하게 처리 해야함... 
		//hotkye 처리가 필요한 경우 gm.win.getActiveWindow() 를 통해서 programCd 를 가져오기에 동일하게 처리 필요 (2024.03.25) 
		if ( programCd == "" ) {
			programCd = url.substring(url.lastIndexOf("/") + 1, url.lastIndexOf("."));		// '/ui/SP/Promise/asyncAwait.xml' => asyncAwait
		}
		...
		if (layout == "T") {
			var tabObj = { 
				...
				openAction : (refresh === true) ? "exist":"select", // exist 는 기존 탭을 갱신, new 는 항상 새로, select는 동일 id 가 존재하면 선택, last: 기존 tab을 마지막 tab으로 이동후 선택
				label : menuNm 
			};
			...
			var options = {
				...
				windowTitle : menuNm,
				windowId : menuCode,
				openAction : (refresh === true) ? "existWindow":"selectWindow",
				frameMode : "wframe",
	...
	try {
		...
		if (typeof _funcStr == "undefined") {
			...
			if (typeof _funcStr == "string") {
				...
				//위와 동일 (var targetObj = frameWin; ~~ ) 
				//fun = window.WebSquare.util.getGlobalFunction( frameWin.$p.id + _funcStr);
