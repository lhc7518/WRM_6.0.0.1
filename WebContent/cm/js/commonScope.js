	...
	switch (resultData.statusCode) {
		case msgCode.STATUS_ERROR:
			if (resultData.errorCode == "E0001") {
				...
				com.win.alert(alertMsg, function() { gcm.win.reload(); });
 ...
 * @param {String}  options.convertIndex			[default: 없음] type이 "0" 또는 "1"인 경우, 특정 컬럼만 type이 "1" 또는 "0"인 데이터로 다운로드. type="1"인 상태에서 convertIndex="0,2"인 경우, index가 0,2인 컬럼은 컬름은 type="1"로 다운로드.
 ...
 * @param {String}  options.useHeaderDataFormat		[default: 없음] "true"인 경우 Header의 dataType에 따라 Excel 파일에 표시 형식을 출력. dataType="text"인 셀은 Excel의 표시형식에 '텍스트' 출력, dataType="number" 혹은 "bigDecimal" 셀은 "숫자" 출력.
 * @param {String}  options.customToDataType		[default: 없음] "true"인 경우 inputType="custom"인 경우 dataType에 따라 Excel 파일에 표시 형식을 출력. dataType="text"인 셀은 Excel의 표시형식에 '텍스트' 출력, dataType="number" 혹은 "bigDecimal" 셀은 "숫자" 출력.
 ...
 * @param {String} options.lazyInfoArr				[default: false] infoArr 출력 작업 작업 순서 ("false"인 경우 가장 먼저 출력, "true"인 경우 가장 마지막에 출력.)
 * @param {String} options.checkedColumnId			[default: "chk"] Checked Column Id 설정 
 * @param {Boolean} options.checkedData				[default : false] checkedColumnId 옵션에 정의된 컬럼이 체크된 데이터만 엑셀 다운로드를 수행한다. dataHandler 옵션과 동시에 사용할 수 없다. 동시에 설정할 경우 dataHandler만 적용된다.
 * @param {Function} options.dataHandler			[default: 없음] dataHandler속성에 함수을 설정하게 되면 설정된 함수를 호출한다. 함수에서 데이터를 생성하여 1차원 Array로 반환하게 되면 반환된 데이터를 이용하여 엑셀을 생성한다.
 *
...
scwin.dataHanlder = function(gridViewId) {
	...
	return checkedDataList.reduce(function(arr, currentValue) {
		arr = arr.concat(currentValue);
		return arr;
	}, []);
}
 */
com.data.downloadGridViewExcel = function(grdObj, options, infoArr) {
	...
	var opts = {
		...
		useProvider: options.useProvider || "false", // Provider 사용 여부 설정
		useSplitProvider : options.useSplitProvider || "false", // Split Provider 사용 여부 설정
		dataProvider : options.dataProvider || "", // ExcelHeaderDown 설정을 위한 속성
		...
		autoSizeColumn: options.autoSizeColumn || "false", //String, [defalut: false] 너비자동맞춤 설정 유무
		...
		mergeCell : options.mergeCell || "false", //String, [default: false] gridView mergeCell API로 cell 머지시, excel에도 동일하게 머지되어 다운로드 할지 여부		
		...
		useHeaderDataFormat: options.useHeaderDataFormat || "", // [default: 없음] "true"인 경우 Header의 dataType에 따라 Excel 파일에 표시 형식을 출력. dataType="text"인 셀은 Excel의 표시형식에 '텍스트' 출력, dataType="number" 혹은 "bigDecimal" 셀은 "숫자" 출력.
		customToDataType : options.customToDataType || "", // [default: 없음] "true"인 경우 inputType="custom"인 경우 dataType에 따라 Excel 파일에 표시 형식을 출력. dataType="text"인 셀은 Excel의 표시형식에 '텍스트' 출력, dataType="number" 혹은 "bigDecimal" 셀은 "숫자" 출력.
		...
		lazyInfoArr : options.lazyInfoArr || "false", // [default: false] infoArr 출력 작업 작업 순서 ("false"인 경우 가장 먼저 출력, "true"인 경우 가장 마지막에 출력.)
...
//엑셀다운로드 async /await 처리 (2024-01-30)
com.data.downloadGridViewExcel2 = function(grdObj, options, infoArr) {
	return new Promise(function(resolve, reject) {
		
		//options.onSuccessCallback 함수가 없을 경우 resolve 처리 
		if (com.util.isEmpty(options.onSuccessCallback)) {
 ...
 * @param {Number}  options.sheetName			[default: 없음] Excel 파일에서 그리드의 데이터가 있는 sheet명 (SheetNo보다 SheetName이 우선순위가 높음)
 * @param {Boolean} options.activeSheet			[default: false] WGear연계 속성. 활성화 되어 있었던 Sheet를 업로드 한다.
 * @param {Number}  options.sheetName			[default: 없음] Excel 파일에서 그리드의 데이터가 있는 sheet명 (SheetNo보다 SheetName이 우선순위가 높음)
 * @param {Number}  options.startRowIndex		[default: 0] excel파일에서 그리드의 데이터가 시작되는 행의 번호(헤더 포함)
 ...
 * @param {String}  optinos.optionParam			[default: 없음] DRM 연계시 사용자 정의 class에 HashMap 인자로 전달할 값. key는 "optionParam"으로 참조.
 * @param {String}  options.cellDataConvertor	[default: ""] 컬럼값을 사용자가 수정할수 있는 연계 클래스의 전체 패키지명. (AbstractCellDataProvider class를 상속후 convertValue method를 구현해야 함.
 * @memberOf com.data
 * @date 2020.05.16
 * @author Inswave Systems
 * @example
...
com.data.uploadGridViewExcel = function (grdObj, options) {
	...
	var opts = {
		...
		sheetName: options.sheetName || "", // [default: 없음] Excel 파일에서 그리드의 데이터가 있는 sheet명 (SheetNo보다 SheetName이 우선순위가 높음)
		activeSheet: options.activeSheet || false, // [default: false] WGear연계 속성. 활성화 되어 있었던 Sheet를 업로드 한다.
		...
		useMaxByteLength: options.useMaxByteLength || false, // [default: false] ignoreChar 속성으로 설정한 문자를 제외하고 maxByteLength 속성으로 설정한 길이만큼의 데이터만 업로드.
		byteCheckEncoding: options.byteCheckEncoding || "EUC-KR", // [default: EUC-KR] useMaxByteLength 설정되어 있는경우 byte처리시 지정할 인코딩. EUC-KR인경우 2byte처리 UTF-8일경우 3byte 처리. (default는 EUC-KR)
		dateFormat: options.dateFormat || "yyyy-MM-dd", // [default: yyyy-MM-dd] 엑셀의 셀포맷이 날짜형식으로 되어 있는경우 format. 기본값은 "yyyy-MM-dd"
		decimal: options.decimal || 4, // [default: 4] 셀의 데이터가 소수인 경우, 최종 소수점 자리수. (기본값: 4) (예: 3인경우 4자리에서 반올림해서 3자리를 최종 표시.)
		trim: options.trim || 0, // [default: 0] Excel 파일에서 그리드의 데이터에 header가 있는지 여부(1이면 header 존재 0이면 없음)
		...
		removeColumns: options.removeColumns || "", // [default: 없음] 저장 하지 않을 column index, 여러컬럼인 경우 콤마(,)로 구분해서 정의.
		useModalDisable: options.useModalDisable || false, // [default: false] 업로드 팝업창이 활성화 될때 부모창의 컴포넌트 disabled 처리 유무.
		cellDataConvertor: options.cellDataConvertor || "", // [default: ""] 컬럼값을 사용자가 수정할수 있는 연계 클래스의 전체 패키지명. (AbstractCellDataProvider class를 상속후 convertValue method를 구현해야 함.
		optionParam: options.optionParam || undefined, // [default: 없음] DRM 연계시 사용자 정의 class에 HashMap 인자로 전달할 값. key는 "optionParam"으로 참조.
 ...
 * @memberOf com.data
 * @date 2020.05.16
 * @param {Object} grpObj Group 컴포넌트 객체 또는 아이디 (TabControl Tab 내에 Group 컴포넌트는 아이디를 전달해야 함) 
...
if (com.data.validateGroup("grp_memberBasic2", valInfo, tacMenuInfo, "tabMenuInfo1") == false) {
	return false;
}
...
com.data.validateGroup = function (grpObj, valInfoArr, tacObj, tabId) {
	if (!com.util.isEmpty(tacObj) && (tacObj.getPluginName() === "tabControl")) {
	...
	try {
		...
		if (errors.length > 0) { 
            ...
            promise = promise.then(function () {
            	if (typeof tacObj !== 'undefined' && !com.util.isEmpty(tabId)) {                
		...
		function _setResult(dataCollection, objId, valInfo, value) {
			...
			if (com.util.isEmpty(msgInfo.message) === false) {
				...
				if (com.util.isEmpty(valInfo.label) === false) {
					...
					errors[errIdx].columnName = com.data.getColumnName(comObj);
 ...
 */
com.util.getFunction = function(_funcStr, _scopeId) {
...
com.win.getLanguage = function() {
	var language = com.win.getLangCode() || navigator.language || navigator.userLanguage || navigator.systemLanguage;
 ...
 */
com.win.openPopup = function(url, opts, data) {
	return new Promise(function(resolve, reject) {
		...
		if (com.util.isEmpty(data.callbackFn)) {
			data.callbackFn = function(rtn) {
				if (!com.util.isEmpty(rtn)) {
					if (!com.util.isEmpty(rtn.clickValue)) {
						resolve(rtn.clickValue);
					} else {
						resolve(rtn);
					}
 ...
 * @author Inswave Systems
 * @description
1. $p.isPopup() 함수는 팝업으로 오픈한 페이지에서만 true를 반환하고, 팝업 페이지에서 wframe으로 다른 페이지를 include한 경우 wframe으로 로딩한 페이지에서 false를 반환한다.
2. com.win.isPopup() 함수는 팝업 페이지에서 wframe으로 다른 페이지를 include한 경우에도 wframe으로 로딩한 페이지에서 true를 반환한다.
 * @example
if (com.win.isPopup()) {
...
com.win.openMenu = function(menuNm, url, menuCode, paramObj, option) {
	//openMenu 처리 전 체크사항 
	//1. tab, windowContainer, wframe 이 아닌 별도 open(window.open) 여부 체크 
	//2. openMenu 건수 지정 시 건수 체크 (동일id 는 제외하고 건수 체크) 
	
	//1. window.open 으로 open 처리 해야되는 경우 (ex: EXT_YN='Y' 일 경우 window.open) 
	var matchInfo = gcm.menuComp.getMatchedJSON("MENU_CODE", menuCode)[0];
	if ( !com.util.isEmpty(matchInfo) ) {
		if ( !com.util.isEmpty(matchInfo.EXT_YN) ) {
			if ( matchInfo.EXT_YN == "Y" ) {
				if ( !com.util.isEmpty(matchInfo.SRC_PATH) ) {
					//외부 open 
					window.open(matchInfo.SRC_PATH);
					return true;					
				}
			}
		}
	...
	//2. open 건수 체크 필요 시 
	var MAX_MENU_CNT = 10;	//제한 건수 (gcm 에 변수로 지정필요) 
	var currentCnt = 0;	
	var isSame = false;
	var layout = $p.main().scwin.getLayoutId();

	if ( layout == "T" || layout == "M" ) {
		if ( layout == "T" ) {
			currentCnt = $p.main().tac_layout.getTabCount();
			$p.main().tac_layout.tabArr.find(function(item){
				if ( item.userTabID == menuCode) {
					isSame = true;
				}
			});
		} else {
			currentCnt = $p.top().wdc_main.windows.length;	
			$p.top().wdc_main.windows.find(function(item){
				if ( item.windowId == menuCode) {
					isSame = true;
				}
			});
		}
		if ( !isSame ) {
			//동일건이 존재하지 않으므로, 건수 체크 
			if ( currentCnt >= MAX_MENU_CNT ) {
				com.win.alert("최대 화면 갯수("+MAX_MENU_CNT+") 를 초과하였습니다. 기존에 열러있는 화면을 닫고 여시기 바랍니다.");
				return false;
			}
		}
	}
...
//메뉴cd 에 해당하는 화면 open 
com.win.openMenuCd = function(menuCd, paramObj) {
	var matchInfo = gcm.menuComp.getMatchedJSON("MENU_CD", menuCd)[0];
	if ( !com.util.isEmpty(matchInfo) ) {
		return com.win.openMenu( matchInfo.MENU_NM, matchInfo.SRC_PATH, matchInfo.MENU_CD, paramObj, {isHistory : true} );	//history 저장 
	}
};
 ...
 */
com.num.parseInt = function (value, defaultValue) {
 ...
 */
com.num.parseFloat = function (value, defaultValue) {
 ...
 * com.hkey.setCKEditorShortKeyDownAction 함수는 CkEditor를 포함한 화면 오픈 시 호출해야 한다.
 * 
 * @memberOf com.hkey
 * @date 2021.09.04
 * @author Inswave Systems
 */
com.hkey.setCkEditorShortKeyDownAction = function() {
...
com.hkey.setData = function(shortKeyArray) {
	if ( shortKeyArray.length > 0 ) {
		...
		for ( var i in shortKeyArray ) {
			var data = {};
			data.EVENT_TYPE = "F";		//function(F), buttion(B) 
			data.EVENT_YN = "Y";
